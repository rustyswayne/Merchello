...
namespace Merchello.Core.Persistence
{
    ...
    /// </summary>
    internal static class NPocoDatabaseExtensions
    {
        // NOTE
        //
        ...
        public static void BulkInsertRecords<T>(this IDatabase db, ISqlSyntaxProviderAdapter sqlSyntax, IEnumerable<T> collection)
        {
            //don't do anything if there are no records.
            var pocos = collection as T[] ?? collection.ToArray();
            if (pocos.Any() == false)
                return;

            using (var tr = db.GetTransaction())
            {
                db.BulkInsertRecords(sqlSyntax, pocos, tr, true);
            }
        }
        ...
        public static void BulkInsertRecords<T>(this IDatabase db, ISqlSyntaxProviderAdapter sqlSyntax, IEnumerable<T> collection, ITransaction tr, bool commitTrans = false)
        {
            //don't do anything if there are no records.
            var pocos = collection as T[] ?? collection.ToArray();
            if (pocos.Any() == false)
            ...
            try
            {
                ...
                else
                {
                    ...
                    var cmds = db.GenerateBulkInsertCommand(pocos, db.Connection, out sqlStatements);
        ...
        public static void TruncateTable(this IDatabase db, ISqlSyntaxProviderAdapter sqlSyntax, string tableName)
        {
