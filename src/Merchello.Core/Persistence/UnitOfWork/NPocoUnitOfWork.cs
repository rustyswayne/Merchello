namespace Merchello.Core.Persistence.UnitOfWork
{
    ...
    internal class NPocoUnitOfWork : UnitOfWorkBase, IDatabaseUnitOfWork
    {
        ...
        /// </summary>
        /// <param name="dbAdapter">A database.</param>
        ...
        internal NPocoUnitOfWork(IDatabaseAdapter dbAdapter, RepositoryFactory factory)
            : base(factory)
        {
        ...
        public void ReadLock(params int[] lockIds)
        {
            ...
            if (this.DatabaseAdapter.Database.Transaction.IsolationLevel < IsolationLevel.RepeatableRead)
                throw new InvalidOperationException("A transaction with minimum RepeatableRead isolation level is required.");

            // *not* using a unique 'WHERE IN' query here because the *order* of lockIds is important to avoid deadlocks
            foreach (var lockId in lockIds)
            {
                var i = this.DatabaseAdapter.Database.ExecuteScalar<int?>(
        ...
        public void WriteLock(params int[] lockIds)
        {
            ...
            foreach (var lockId in lockIds)
            {
                var i = this.DatabaseAdapter.Database.Execute(
        ...
        public override void Begin()
        {
            ...
            if (this._transaction == null)
                this._transaction = this.DatabaseAdapter.Database.GetTransaction();
        }
