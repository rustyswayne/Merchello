namespace Merchello.Core.Strategies.Packaging
{
    ...
    public class DefaultWarehousePackagingStrategy : PackagingStrategyBase
    {
        ...
        public override IEnumerable<IShipment> PackageShipments()
        {
            ...
            var quoted = MC.Services.ShipmentService.GetShipmentStatusByKey(Constants.ShipmentStatus.Quoted);
            ...
            var warehouse = MC.Services.WarehouseService.GetDefaultWarehouse();
            ...
            var variants =
                   MC.Services.ProductService.GetAllProductVariants(
                       shippableVisitor.ShippableItems
                       .Select(x => x.ExtendedData.GetProductVariantKey())
                       .Where(x => !Guid.Empty.Equals(x)).ToArray()).ToArray();
