namespace Merchello.Core.ValueConverters
{
    ...
    internal class DetachedPublishedPropertyValueConverter
    {
        ...
        /// 
        /// </remarks>
        public object ConvertDbToEditor(IContentType contentType, KeyValuePair<string, string> detachedContentValue)
        {
            if (contentType == null || !_ready) return JToken.FromObject(detachedContentValue.Value);
            ...
            var prop = new Property(propType, detachedContentValue.Value);

            // Lookup the property editor
            var propEditor = PropertyEditorResolver.Current.GetByAlias(propType.PropertyEditorAlias);

            // Get the editor to do it's conversion
            return propEditor.ValueEditor.ConvertDbToEditor(prop, propType, _dataTypeService);
        }
