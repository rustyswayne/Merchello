    ...
    public class FakeHttpContextFactory
	{
		...
		public FakeHttpContextFactory(string path)
		{
			if (path.StartsWith("http://") || path.StartsWith("https://"))
				this.CreateContext(new Uri(path));
			else
				this.CreateContext(new Uri("http://mysite" + VirtualPathUtility.ToAbsolute(path, "/")));
		}
		...
		{
			if (path.StartsWith("http://") || path.StartsWith("https://"))
				this.CreateContext(new Uri(path), routeData);
			else
				this.CreateContext(new Uri("http://mysite" + VirtualPathUtility.ToAbsolute(path, "/")), routeData);
		}
		...
		private void CreateContext(Uri fullUrl, RouteData routeData = null)
		{
            ...
            this.RequestContext = requestContextMock.Object;
            ...
            requestMock.Setup(x => x.RequestContext).Returns(this.RequestContext);
